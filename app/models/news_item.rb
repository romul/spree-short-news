class NewsItem < ActiveRecord::Base
  ...
  def self.last_news(identifier, limit = false)
    if limit
      ...
      identifier, Time.now, Time.now], :order=>"published_start_at DESC, created_at DESC", :limit => limit)
    else
      NewsItem.all(:conditions=>
      ["identifier=? AND (published_start_at<? OR published_start_at IS NULL) AND (published_end_at>? OR published_end_at IS NULL)",
      identifier, Time.now, Time.now], :order=>"published_start_at DESC, created_at DESC")
    end
  end
